 ...
 */
let ChatAgent = ChatAgent_1 = class ChatAgent extends Agent {
    constructor(input) {
        super(input);
    }
    _agentType() {
    ...
    static fromLLMAndTools(llm, tools, args) {
        ChatAgent_1.validateTools(tools);
        const prompt = ChatAgent_1.createPrompt(tools, args);
        const chain = new LLMChain({ prompt, llm });
        return new ChatAgent_1({
            llmChain: chain,
...
ChatAgent = ChatAgent_1 = __decorate([
    (staticImplements)
], ChatAgent);
export { ChatAgent };
